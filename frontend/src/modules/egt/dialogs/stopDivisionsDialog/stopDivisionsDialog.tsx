...
export type StopDivisionDialogProps = {
  isOpen: boolean;
  onClose: () => void;
  divisions: EgtDivision[];
};

export function StopDivisionsDialog(props: StopDivisionDialogProps) {
  ...
  return (
    <Dialog open={props.isOpen} fullScreen={fullScreen} maxWidth="sm" fullWidth>
      <DialogTitle>
        {t("stop", { name: t("division", { ns: "egt" }) })}
      </DialogTitle>
      <DialogContent>
        <Typography>{t("stop_confirmation", { ns: "common" })}</Typography>
        <List>
